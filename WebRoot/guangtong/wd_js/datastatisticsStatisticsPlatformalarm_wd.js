$(document).ready(function() {
	...
	function getPlatformAlarmsData() {
		var data_json = {
			"currentPage": tpa,
			"startTime": times,
			"endTime": timee,
		...
		$.ajax(config.url + "charts/getPlatformAlarmsData", {
			data: data_json,
			dataType: "json",
			type: "POST",
			timeout: 10000,
			success: function(data) {
				if(data.success == true) {
					...
					for(var i = 0; i < data.beanList.length; i++) {
						...
						if(data.beanList[i].quantity == null){
							html += '<td></td>';
						}else{
							html += '<td>' + data.beanList[i].quantity + '</td>';
						};
						html += '<td><a class="button blue" style="margin:0 10px;padding:0 12px;line-height:24px;" charType="pie" charName="Platformalarm" simNo="' + data.beanList[i].simNo + '">查看统计图</a></td>';
						html += '</tr>';
					}
					$(".list_xk tbody").html(html);
					fenye_lv(data.tp, function() {
						getPlatformAlarmsData();
					});
					highcharClick();
				}
			}
	...
	function exportExcel() {
		$(".listgrp_xk").on("click",".inquirybox_xk .inquiry_xk a.button:eq(1)",function() {
			...
			location.href = config.url + "charts/getPlatChartAlarmsExcel?simNos=" + vId + "&startTime=" + $(".time_plugin_lv_onclick:eq(0)").val() + "&endTime=" + $(".time_plugin_lv_onclick:eq(1)").val();
		});
	}
	exportExcel();
	getCarList();
