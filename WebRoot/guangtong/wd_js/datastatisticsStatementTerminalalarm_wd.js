$(document).ready(function() {
	...
	//报警列表
	function callList() {
		$.ajax(config.url + "forms/getAllTerminalFormAlarm", {
			dataType: "json",
			type: "POST",
			timeout: 10000,
			success: function(data) {
				if(data.success == true) {
					data.obj == null ? data = data.t : data = data.obj;
					for(one in data.terminal) {
						if(data.terminal[one] != null) {
							html = "";
							html += '<option value="' + data.terminal[one] + '">' + data.terminal[one] + '</option>';
							$(".select").append(html);
						}
					}
				}
	...
	function getTerminalFormsByPage() {
		$.ajax(config.url + "forms/getTerminalFormsByPage", {
			data: {
				"currentPage": tpa,
				"startTime": times,
				"endTime": timee,
	...
	function exportExcel() {
		$(".inquirybox_xk .inquiry_xk a.button:eq(1)").click(function() {
			...
			location.href = config.url + "forms/getTerFormsByPageExcel?simNos=" + vId + "&startTime=" + $(".time_plugin_lv_onclick:eq(0)").val() + "&endTime=" + $(".time_plugin_lv_onclick:eq(1)").val()+"&type="+ $(".inquiry_xk select option:selected").val()
;
		});
	}
	exportExcel();
